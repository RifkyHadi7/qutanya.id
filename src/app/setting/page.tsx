...
export default function SettingsPage() {
  ...
  const updateBiodata = async () => {
    try {
      const response = await axios.post("https://qutanya-be.vercel.app/user/update", {
        nama: name,
        pekerjaan: job,
      }, {
        headers: {
      ...
      if (data.status === "success") {
        ...
        const updatedUserData = { ...userData, nama: name, biodata: [{ ...userData.biodata[0], pekerjaan: job }] };
        sessionStorage.setItem("userData", JSON.stringify({ data: updatedUserData }));
